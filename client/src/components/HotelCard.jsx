...
const HotelCard = ({room, index}) => {
  ...
  return (
        ...
        text-gray-800 font-medium rounded-full'>{t("hotel.bestSeller")}</p>}
        ...
        <div className='p-4 pt-5'>
            ...
            <div className='flex items-center justify-between mt-4'>
                <p><span className='text-xl text-gray-800'>${room.pricePerNight}</span>/{t("hotel.perNight")}</p>
                <button className='px-4 py-2 text-sm font-medium border border-gray-300 
                rounded hover:bg-gray-50 transition-all cursor-pointer'>{t("hotel.bookNow")}</button>
            </div>
        </div>
        </Link>
  )
