...
const MyBookings = () => {
  ...
  return (
    <div className='py-28 md:pb-35 md:pt-32 px-4 md:px-16 lg:px-24 xl:px-32'>
        <Title title={t("bookings.title")} subTitle={t("bookings.subtitle")}
        align='left'/>

        <div className='max-w-6xl mt-8-w-full text-gray-800'>
            <div className='hidden md:grid md:grid-cols-[3fr_2fr_1fr] w-full border-b border-gray-300 font-medium text-base py-3'>
                <div className='w-1/3'>{t("bookings.hotels")}</div>
                <div className='w-1/3'>{t("bookings.date")}</div>
                <div className='w-1/3'>{t("bookings.payment")}</div>
            </div>
                    ...
                    <div className='flex flex-col md:flex-row'>
                        ...
                        <div className='flex flex-col gap-1.5 max-md:mt-3 min-md:ml-4'>
                            ...
                            <div className='flex items-center gap-1 text-sm text-gray-500'>
                                ...
                                <span>{t("bookings.guests")} {booking.guests}</span>
                            </div>
                            <p className='text-base'>{t("bookings.total")}: ${booking.totalPrice}</p>
                        </div>
                    </div>
                    {/* Date & Timings */}
                    <div className='flex flex-row md:items-center md:gap-12 mt-3 gap-8'>
                        <div>
                            <p>{t("bookings.checkIn")}:</p>
                            <p className='text-gray-500 text-sm'>
                                {new Date(booking.checkInDate).toDateString()}
                            </p>
                        </div>
                        <div>
                            <p>{t("bookings.checkOut")}:</p>
                            <p className='text-gray-500 text-sm'>
